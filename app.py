...
# --- 1. OYLAMA SİSTEMİ ALTYAPISI (YENİ EKLENDİ) ---
VOTE_FILE = 'votes.json'

def load_votes():
    # Dosya yoksa boş bir sözlük oluştur
...
def save_votes(votes):
    # Puanları dosyaya kaydet
    with open(VOTE_FILE, 'w') as f:
        json.dump(votes, f)

# Uygulama açılınca oyları yükle
votes_db = load_votes()
...
# --- 2. OY VERME API'si (YENİ EKLENDİ) ---
...
def submit_vote():
    ...
    # İlk defa oy veriliyorsa kayıt aç
            ...
            try:
                ...
                for x in items:
                    if x not in seen:
                        ...
                        seen.add(x)
                clean_items = clean_items[:9] # Senin ayarın (9 item)

                kda_text = kda_div.text.strip()
                result = "lose"
                if "Victory" in row.text or "Zafer" in row.text: result = "win"
                
                # --- 3. PUAN VERİSİNİ EKLE (YENİ KISIM) ---
                # Maç Kimliği Oluştur (İsim + Şampiyon + KDA)
                match_id = f"{summoner_name}-{champ_key}-{kda_text}".replace(" ", "")
                
                current_score = "-"
                vote_count = 0
                
                # Veritabanından (JSON) puanı çek
                ...
                matches_info.append({
                    "match_id": match_id,        # Frontend için gerekli
                    "champion": champ_key,
                    "result": result,
                    "kda": kda_text,
                    "img": final_champ_img,
                    "items": clean_items,
                    "user_score": current_score, # Mevcut puan
